...
export default function Home() {
    ...
    const base64Hint = useMemo(() => {
        const hint = "第3問のヒント: 新しい元号。2019年に始まったよ。";
    ...
    // 謎3に文字が入ったら、暗号化された投稿を一度だけ表示
    useEffect(() => {
        if (threeIsAnswered && posts.length === 1 && base64Hint) {
            setPosts((prev) => ([
                ...prev,
    ...
    return (
        <div className="container" style={{ maxWidth: 900, margin: "40px auto", padding: "32px", display: "flex", gap: "32px" }}>
            {/* 左：SNS風ヒント */}
            <div style={{ width: 320, background: "#f6f8fb", borderRadius: "16px", boxShadow: "0 6px 18px rgba(0,0,0,0.08)", padding: "20px 14px", display: "flex", flexDirection: "column", alignItems: "center" }}>
                ...
                {hasDecrypted && (
                    <div style={{ marginTop: 10, color: "#6b7280", fontSize: "0.85rem" }}>復号回数（第3問）: {decryptCounts[3] ?? 1}</div>
                )}
            </div>

            {/* 右：UIダミー */}
            <div style={{ flex: 1, background: "#fff", borderRadius: "16px", boxShadow: "0 8px 32px rgba(0,0,0,0.12)", padding: "32px" }}>
                <h1 style={{ textAlign: "center", marginBottom: 32, fontWeight: 700, color: "#2c3e50" }}>謎解きチャレンジ</h1>
                {[item.quiz_one, item.quiz_two, item.quiz_three, item.quiz_four].map((quiz, idx) => (
                    <div key={idx} style={{ marginBottom: 28 }}>
                        <h2 dangerouslySetInnerHTML={{ __html: quiz }} style={{ fontSize: "1.2rem", color: "#34495e", marginBottom: 12, background: "#f5f7fa", padding: "8px 16px", borderRadius: "8px" }}></h2>
                        <input type="text" className="riddle-input" placeholder={`謎${idx + 1}の答えを入力`} style={{ width: "100%", padding: "10px", fontSize: "1rem", borderRadius: "6px", border: "1px solid #d1d5db", marginBottom: "4px" }} />
                    </div>
                ))}

                <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 16 }}>
                    <input type="text" value={crosswordAnswer} onChange={e => setCrosswordAnswer(e.target.value)} placeholder="クロスワードの答えを入力" style={{ flex: 1, padding: "10px", fontSize: "1rem", borderRadius: "6px", border: "1px solid #d1d5db" }} />
                    <button onClick={handleCheckAnswer} disabled={isLoading} style={{ padding: "10px 20px", background: "#0984e3", color: "#fff", border: "none", borderRadius: "6px", fontWeight: 600, cursor: isLoading ? "not-allowed" : "pointer", boxShadow: "0 2px 8px rgba(0,0,0,0.08)", transition: "background 0.2s", opacity: isLoading ? 0.6 : 1 }}>
                        {isLoading ? "判定中..." : "回答する"}
                    </button>
                </div>

...
export async function POST(request: Request) {
  try {
    ...
    try {
      ...
      console.error("update-room-time: failed to parse JSON body", e);
      return NextResponse.json({ ok: false, error: "invalid_json" }, { status: 400 });
    }
    const bodyTyped: Body = body;
    const { roomId, elapsedTime } = body;
    ...
    const upsertRes: any = await supabaseAdmin
      .from("RoomNum")
